  ...
  const updateOrderStatus = async ({ id, status }: { id: string, status: OrderStatus }) => {
    ...
    try {
      ...
      if (status === 'confirmed' && previousStatus !== 'confirmed') {
        console.log("📦 [useOrders] Reserving stock for confirmed order:", id);
        ...
        if (reservationResult.shortageItems && reservationResult.shortageItems.length > 0) {
          console.warn("⚠️ [useOrders] Stock shortage detected:", reservationResult.shortageItems);
        ...
        if (!reservationResult.success) {
          console.warn("⚠️ [useOrders] Stock reservation had issues:", reservationResult.errors);
      ...
      if (status === 'cancelled' && previousStatus !== 'cancelled') {
        console.log("🔓 [useOrders] Releasing stock reservation for cancelled order:", id);
        const releaseResult = await releaseReservation({ orderId: id });
        
        if (!releaseResult.success) {
          console.warn("⚠️ [useOrders] Stock release had issues:", releaseResult.errors);
        }
      }
    } catch (stockError) {
      // Stok rezervasyon hatası sipariş durumu güncellemesini engellemez
      console.error("❌ [useOrders] Stock reservation error:", stockError);
