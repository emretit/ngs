...
export const useStockReservation = () => {
  ...
  const reserveStock = async ({ orderId }: ReserveStockParams): Promise<StockReservationResult> => {
    console.log("📦 [useStockReservation] Reserving stock for order:", orderId);
    ...
    try {
      ...
      if (!orderItems || orderItems.length === 0) {
        console.log("⚠️ [useStockReservation] No items found for order");
        return result;
      }

      console.log(`📋 [useStockReservation] Found ${orderItems.length} order items`);

      // 2. Process each item
      for (const item of orderItems) {
        // Skip items without product_id
        if (!item.product_id) {
          console.log(`⏭️ [useStockReservation] Skipping item ${item.name} (no product_id)`);
          result.skippedItems++;
        ...
        try {
          ...
          if (!hasStock) {
            ...
            console.warn(`⚠️ [useStockReservation] Insufficient stock for ${item.name}`);
            continue;
          }
          ...
          console.log(`✅ [useStockReservation] Reserved ${item.quantity} units of ${item.name}`);

        } catch (itemError) {
          const errorMsg = itemError instanceof Error ? itemError.message : "Bilinmeyen hata";
          result.errors.push(`${item.name}: ${errorMsg}`);
          result.success = false;
          console.error(`❌ [useStockReservation] Error processing item ${item.name}:`, itemError);
        }
      }

      console.log(`✅ [useStockReservation] Reservation complete:`, result);
      return result;

    } catch (error) {
      console.error("❌ [useStockReservation] Error reserving stock:", error);
      result.success = false;
  ...
  const releaseReservation = async ({ orderId }: ReserveStockParams): Promise<StockReservationResult> => {
    console.log("🔓 [useStockReservation] Releasing stock reservation for order:", orderId);
    ...
    try {
      ...
      console.log(`📋 [useStockReservation] Found ${orderItems.length} order items to release`);

      // 2. Process each item
      for (const item of orderItems) {
        // Skip items without product_id or not reserved
        if (!item.product_id || item.stock_status !== 'in_stock') {
          console.log(`⏭️ [useStockReservation] Skipping item ${item.name} (no reservation)`);
          result.skippedItems++;
          continue;
        }
        ...
        try {
          ...
          result.reservedItems++;
          console.log(`✅ [useStockReservation] Released ${item.quantity} units of ${item.name}`);

        } catch (itemError) {
          const errorMsg = itemError instanceof Error ? itemError.message : "Bilinmeyen hata";
          result.errors.push(`${item.name}: ${errorMsg}`);
          result.success = false;
          console.error(`❌ [useStockReservation] Error releasing item ${item.name}:`, itemError);
        }
      }

      console.log(`✅ [useStockReservation] Release complete:`, result);
      return result;

    } catch (error) {
      console.error("❌ [useStockReservation] Error releasing reservation:", error);
      result.success = false;
      result.errors.push(error instanceof Error ? error.message : "Bilinmeyen hata");
      return result;
    }
  ...
  const reserveStockMutation = useMutation({
    ...
    onError: (error) => {
      console.error("❌ [useStockReservation] Mutation error:", error);
