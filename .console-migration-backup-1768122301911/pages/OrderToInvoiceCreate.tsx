...
const OrderToInvoiceCreate = () => {
  ...
  const loadOrderData = useCallback(async (id: string) => {
    console.log("🔵 [OrderToInvoiceCreate] Loading order data...", { orderId: id });
    try {
      setLoadingData(true);
      setError(null);
      ...
      console.log("✅ [OrderToInvoiceCreate] Order data loaded successfully", { 
        orderNumber: order.order_number,
        itemsCount: order.items?.length || 0 
      });
    } catch (error) {
      console.error("❌ [OrderToInvoiceCreate] Error loading order:", error);
  ...
  const handleSave = async () => {
    console.log("💾 [OrderToInvoiceCreate] Saving invoice...");
    try {
      // Validation
      ...
      console.log("✅ [OrderToInvoiceCreate] Invoice saved successfully", { invoiceId: invoice.id });
      
      toast.success("Fatura başarıyla oluşturuldu");
      navigate(`/sales-invoices/${invoice.id}`);

    } catch (error) {
      console.error('❌ [OrderToInvoiceCreate] Error saving invoice:', error);
