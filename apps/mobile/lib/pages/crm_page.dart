...
import 'crm/crm_common_widgets.dart';
...
class _CrmPageState extends ConsumerState<CrmPage> with SingleTickerProviderStateMixin {
        ...
        slivers: [
          ...
          activitiesAsync.when(
            data: (activities) {
              ...
              if (filtered.isEmpty) {
                return SliverFillRemaining(
                  hasScrollBody: false,
                  child: buildEmptyState('Aktivite bulunamadı', CupertinoIcons.calendar),
                );
              }
            ...
            error: (_, __) => SliverFillRemaining(
              hasScrollBody: false,
              child: buildErrorState('Aktiviteler yüklenemedi'),
            ),
          ),
        ],
      ),
      ...
      child: CustomScrollView(
        slivers: [
          ...
          opportunitiesAsync.when(
            ...
            error: (_, __) => SliverFillRemaining(
              hasScrollBody: false,
              child: buildErrorState('Fırsatlar yüklenemedi'),
            ),
          ),
        ],
      ),
      ...
      child: CustomScrollView(
        slivers: [
          ...
          proposalsAsync.when(
            ...
            error: (_, __) => SliverFillRemaining(
              hasScrollBody: false,
              child: buildErrorState('Teklifler yüklenemedi'),
            ),
          ),
        ],
      ),
      ...
      child: CustomScrollView(
        slivers: [
          ...
          ordersAsync.when(
            ...
            error: (_, __) => SliverFillRemaining(
              hasScrollBody: false,
              child: buildErrorState('Siparişler yüklenemedi'),
            ),
          ),
        ],
      ),
  ...
  Widget _buildOpportunityCard(Opportunity opportunity) {
    final statusColor = getOpportunityStatusColor(opportunity.status);
                      ...
                      Row(
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(
                              getOpportunityStatusName(opportunity.status),
                              style: TextStyle(
                                fontSize: 11,
                                fontWeight: FontWeight.w600,
                                color: statusColor,
  ...
  Widget _buildProposalCard(Proposal proposal) {
    final statusColor = getProposalStatusColor(proposal.status);
  ...
  Widget _buildActivityStats(List<Activity> activities) {
    ...
    return Row(
      children: [
        Expanded(child: buildStatBubble('${activities.length}', 'Toplam', CupertinoIcons.circle)),
        const SizedBox(width: 10),
        Expanded(child: buildStatBubble('$todo', 'Yapılacak', CupertinoIcons.circle)),
        const SizedBox(width: 10),
        Expanded(child: buildStatBubble('$inProgress', 'Devam', CupertinoIcons.clock)),
        const SizedBox(width: 10),
        Expanded(child: buildStatBubble('$completed', 'Bitti', CupertinoIcons.checkmark_circle)),
      ],
    );
  }
  ...
  Widget _buildOpportunityStats(List<Opportunity> opportunities) {
    ...
    return Row(
      children: [
        Expanded(child: buildStatBubble('$total', 'Toplam', CupertinoIcons.star)),
        const SizedBox(width: 10),
        Expanded(child: buildStatBubble('$active', 'Aktif', CupertinoIcons.bolt)),
        const SizedBox(width: 10),
        Expanded(child: buildStatBubble('$won', 'Kazanılan', CupertinoIcons.checkmark_circle)),
        const SizedBox(width: 10),
        Expanded(child: buildStatBubble(formatCurrency(totalValue), 'Değer', CupertinoIcons.money_dollar_circle)),
      ],
    );
  }
  ...
  Widget _buildProposalStats(List<Proposal> proposals) {
    ...
    return Row(
      children: [
        Expanded(child: buildStatBubble('$total', 'Toplam', CupertinoIcons.doc)),
        const SizedBox(width: 10),
        Expanded(child: buildStatBubble('$draft', 'Taslak', CupertinoIcons.doc_text)),
        const SizedBox(width: 10),
        Expanded(child: buildStatBubble('$sent', 'Gönderildi', CupertinoIcons.paperplane)),
        const SizedBox(width: 10),
        Expanded(child: buildStatBubble('$accepted', 'Kabul', CupertinoIcons.checkmark_circle)),
      ],
    );
  }
  ...
  Widget _buildOrderStats(List<Order> orders) {
    ...
    return Row(
      children: [
        Expanded(child: buildStatBubble('$total', 'Toplam', CupertinoIcons.cart)),
        const SizedBox(width: 10),
        Expanded(child: buildStatBubble('$pending', 'Bekliyor', CupertinoIcons.clock)),
        const SizedBox(width: 10),
        Expanded(child: buildStatBubble('$processing', 'İşleniyor', CupertinoIcons.gear)),
        const SizedBox(width: 10),
        Expanded(child: buildStatBubble('$completed', 'Tamam', CupertinoIcons.checkmark_circle)),
      ],
    );
  }
