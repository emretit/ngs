...
export const fetchPurchaseRequests = async (filters: {
  ...
  logger.debug("Fetching purchase requests", filters);
...
export const fetchPurchaseRequestById = async (id: string): Promise<PurchaseRequest> => {
  logger.debug(`Fetching purchase request with ID: ${id}`);
  ...
  try {
    ...
    if (error) {
      logger.error(`Error fetching purchase request ID ${id}`, error);
...
export const fetchRequestWithItems = async (id: string) => {
  logger.debug(`Fetching purchase request with items for ID: ${id}`);
  ...
  try {
    ...
    logger.info(`Successfully fetched purchase request ID ${id} with ${items.length} items`);
    return { ...request, items };
  } catch (error) {
    logger.error(`Exception in fetchRequestWithItems for ID ${id}`, error);
...
export const createPurchaseRequest = async (requestData: PurchaseRequestFormData) => {
  logger.info("Creating new purchase request", { title: requestData.title, itemCount: requestData.items?.length || 0 });
  ...
  try {
    ...
    if (requestError) {
      logger.error("Error creating purchase request", requestError);
    ...
    logger.info(`Successfully created purchase request ID ${(request as any).id}`);
    toast.success("Satın alma talebi başarıyla oluşturuldu");
    return request;
  } catch (error) {
    logger.error("Exception in createPurchaseRequest", error);
...
export const updatePurchaseRequest = async ({ id, data }: { id: string, data: Partial<PurchaseRequestFormData> }) => {
  logger.info(`Updating purchase request ID ${id}`, { 
    title: data.title, 
  ...
  try {
    ...
    if (requestError) {
      logger.error(`Error updating purchase request ID ${id}`, requestError);
    ...
    if (items && items.length > 0) {
      ...
      logger.info(`Updated ${items.length} items for purchase request ID ${id}`);
    }

    logger.info(`Successfully updated purchase request ID ${id}`);
    toast.success("Satın alma talebi başarıyla güncellendi");
    return { id };
  } catch (error) {
    logger.error(`Exception in updatePurchaseRequest for ID ${id}`, error);
...
export const deletePurchaseRequest = async (id: string) => {
  logger.info(`Deleting purchase request ID ${id}`);
  ...
  try {
    ...
    if (error) {
      logger.error(`Error deleting purchase request ID ${id}`, error);
      toast.error("Satın alma talebi silinirken hata oluştu");
      throw error;
    }

    logger.info(`Successfully deleted purchase request ID ${id}`);
    toast.success("Satın alma talebi başarıyla silindi");
    return { id };
  } catch (error) {
    logger.error(`Exception in deletePurchaseRequest for ID ${id}`, error);
