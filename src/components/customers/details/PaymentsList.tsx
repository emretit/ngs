...
export const PaymentsList = ({ customer, onAddPayment }: PaymentsListProps) => {
  ...
  // Alış faturaları (müşteriden alınan faturalar customer_id ile, müşteri aynı zamanda tedarikçi ise supplier_id ile)
  ...
  const allTransactions = useMemo<UnifiedTransaction[]>(() => {
    ...
    salesInvoices.forEach((invoice: any) => {
      ...
      transactions.push({
        ...
        branch: invoice.warehouse_id ? 'PERPA' : undefined, // Şube bilgisi varsa
      });
    });
  ...
  const transactionsWithBalance = useMemo(() => {
    ...
    let initialBalance = customer.balance || 0;
  ...
  }, [filteredTransactions, customer.balance]);
