...
import { GeminiUsageTracker, UsageStats } from "@/services/geminiUsageTracker";

interface GeminiUsageMonitorProps {
  isVisible: boolean;
  onToggle: () => void;
}

export default function GeminiUsageMonitor({ isVisible, onToggle }: GeminiUsageMonitorProps) {
  const [stats, setStats] = useState<UsageStats | null>(null);
  const [percentages, setPercentages] = useState({ daily: 0, rateLimit: 0, monthly: 0 });
  const [lastRefresh, setLastRefresh] = useState<Date>(new Date());

  const tracker = GeminiUsageTracker.getInstance();
    ...
    <Card className="mb-4">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-sm font-medium">Gemini API Kullanım İstatistikleri</CardTitle>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={refreshStats} className="h-7 px-2">
              <RefreshCw className="h-3 w-3" />
            </Button>
            <Button variant="outline" size="sm" onClick={handleExport} className="h-7 px-2">
      ...
      <CardContent className="pt-0 space-y-4">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge variant="secondary" className={percentages.daily < 80 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}>
                %{percentages.daily}
              </Badge>
            </div>
          </div>
          <Progress value={percentages.daily} className="h-2" />
        </div>
        ...
        <div className="space-y-2">
          ...
          <Progress value={percentages.rateLimit} className="h-2" />
        </div>

        <div className="flex items-center justify-between pt-2 border-t">
          <span className="text-xs text-muted-foreground">Son: {lastRefresh.toLocaleTimeString('tr-TR')}</span>
          <div className="flex items-center gap-1">
            {percentages.daily < 90 ? (
              <>
