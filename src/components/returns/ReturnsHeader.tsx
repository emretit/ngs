...
import { Plus, RotateCcw, Clock, AlertCircle, CheckCircle, XCircle, Check } from "lucide-react";
...
const ReturnsHeader = ({ returns = [], onCreateReturn }: ReturnsHeaderProps) => {
  // Toplam iade sayısı
  const totalCount = returns.length;

  // Durum kartları için veri
  const statusCards = [
    { status: 'pending', label: 'Bekleyen', color: 'bg-orange-100 text-orange-800 border-orange-200', icon: Clock },
    { status: 'under_review', label: 'İncelenen', color: 'bg-blue-100 text-blue-800 border-blue-200', icon: AlertCircle },
    { status: 'approved', label: 'Onaylanan', color: 'bg-green-100 text-green-800 border-green-200', icon: CheckCircle },
    { status: 'rejected', label: 'Reddedilen', color: 'bg-red-100 text-red-800 border-red-200', icon: XCircle },
    { status: 'completed', label: 'Tamamlanan', color: 'bg-emerald-100 text-emerald-800 border-emerald-200', icon: Check }
  ];

  // Durum sayılarını hesapla
  const statusCounts = statusCards.reduce((acc, { status }) => {
    acc[status] = returns.filter(r => r.status === status).length;
    return acc;
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground/70">
            Ürün iadelerini yönetin ve takip edin.
          </p>
        </div>
      </div>
