...
const SalesInvoicesHeader = ({ invoices = [] }: SalesInvoicesHeaderProps) => {
  ...
  const documentTypeCards = [
    { type: 'e_fatura', icon: FileUp, label: 'e-Fatura', color: 'bg-blue-100 text-blue-800 border-blue-200' },
    { type: 'e_arsiv', icon: FileUp, label: 'e-Arşiv', color: 'bg-purple-100 text-purple-800 border-purple-200' },
    { type: 'fatura', icon: FileUp, label: 'Fatura', color: 'bg-gray-100 text-gray-800 border-gray-200' },
    { type: 'irsaliye', icon: FileUp, label: 'İrsaliye', color: 'bg-amber-100 text-amber-800 border-amber-200' },
    { type: 'makbuz', icon: FileUp, label: 'Makbuz', color: 'bg-green-100 text-green-800 border-green-200' },
    { type: 'serbest_meslek_makbuzu', icon: FileUp, label: 'SMM', color: 'bg-indigo-100 text-indigo-800 border-indigo-200' }
  ];

  // Belge tipi sayılarını hesapla
  const documentTypeCounts = documentTypeCards.reduce((acc, { type }) => {
    acc[type] = invoices.filter(invoice => invoice.document_type === type).length;
    return acc;
    ...
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-3 pl-12 bg-white rounded-md border border-gray-200 shadow-sm">
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground/70">
            Tüm satış faturalarınızı yönetin ve takip edin.
          </p>
        </div>
