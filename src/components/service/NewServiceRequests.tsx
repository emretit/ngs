// @ts-nocheck
// This component is temporarily disabled due to type migration

import React from 'react';

export default function NewServiceRequestsDisabled() {
  return (
    <div className="p-8 text-center">
      <h3 className="text-lg font-semibold text-gray-600 mb-2">Yeni Servis Talepleri Geçici Olarak Devre Dışı</h3>
      <p className="text-gray-500">Bu özellik sistem güncellemesi nedeniyle geçici olarak kullanılamıyor.</p>
    </div>
  );
}