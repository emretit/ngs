...
import { useVknToSupplier } from "@/hooks/useVknToSupplier";
...
const SupplierBasicInfo = ({ formData, setFormData }: SupplierBasicInfoProps) => {
  ...
  const { createSupplierFromVkn, isCreating } = useVknToSupplier();
  ...
  const handleSaveAsSupplier = async () => {
    ...
    await createSupplierFromVkn(vknData);
  };
            ...
            <div className="flex items-center justify-between mb-1">
              ...
              <div className="flex items-center gap-1.5">
                <Button
                  ...
                  onClick={() => {
                    if (mukellefInfo) {
                      setFormData((prevFormData) => ({
                        ...prevFormData,
                        company: mukellefInfo.companyName || prevFormData.company,
                        tax_office: mukellefInfo.taxOffice || prevFormData.tax_office,
                        address: mukellefInfo.address || prevFormData.address,
                        city: mukellefInfo.city || prevFormData.city,
                        district: mukellefInfo.district || prevFormData.district,
                        einvoice_alias_name: mukellefInfo.aliasName || prevFormData.einvoice_alias_name,
                        einvoice_document_type: mukellefInfo.documentType || prevFormData.einvoice_document_type,
                      }));
                    }
                  }}
                ...
                <Button
                  ...
                  disabled={isCreating}
                  className="h-6 px-2 text-xs bg-white hover:bg-green-50 border-green-300 text-green-700 hover:text-green-800"
                >
                  {isCreating ? (
                    <>
