...
export const useAutoPurchaseRequest = () => {
  ...
  const createPurchaseRequest = async ({ orderId, shortageItems }: CreatePurchaseRequestParams) => {
    logger.debug("🛒 [useAutoPurchaseRequest] Creating purchase request for order:", orderId);
    ...
    if (prError) {
      logger.error("❌ [useAutoPurchaseRequest] Purchase request creation error:", prError);
      throw new Error(`Satın alma talebi oluşturulamadı: ${prError.message}`);
    }

    logger.debug("✅ [useAutoPurchaseRequest] Purchase request created:", purchaseRequest.id);
    ...
    if (itemsError) {
      logger.error("❌ [useAutoPurchaseRequest] Items creation error:", itemsError);
      throw new Error(`Satın alma talebi kalemleri eklenemedi: ${itemsError.message}`);
    }

    logger.debug(`✅ [useAutoPurchaseRequest] Added ${requestItems.length} items to purchase request`);
  ...
  const mutation = useMutation({
    ...
    onError: (error) => {
      logger.error("❌ [useAutoPurchaseRequest] Mutation error:", error);
