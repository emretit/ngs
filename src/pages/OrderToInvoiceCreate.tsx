...
import { FileText, Eye, MoreHorizontal, Save, Send, Printer, ShoppingCart, Loader2, AlertCircle } from "lucide-react";
...
const OrderToInvoiceCreate = () => {
  ...
  // Load customer data when watchCustomerId changes
  useEffect(() => {
    let isMounted = true;
  ...
  const loadOrderData = useCallback(async (id: string) => {
    logger.debug("🔵 [OrderToInvoiceCreate] Loading order data...", { orderId: id });
    try {
      setLoadingData(true);
      setError(null);
      ...
      logger.debug("✅ [OrderToInvoiceCreate] Order data loaded successfully", { 
        orderNumber: order.order_number,
        itemsCount: order.items?.length || 0 
      });
    } catch (error) {
      logger.error("❌ [OrderToInvoiceCreate] Error loading order:", error);
      setError(error instanceof Error ? error.message : "Sipariş verileri yüklenirken hata oluştu");
  ...
  useEffect(() => {
    if (!orderId) {
      setError("Sipariş ID'si bulunamadı. Lütfen sipariş sayfasından faturaya çevirin.");
      return;
    }
  ...
  const handleSave = async () => {
    logger.debug("💾 [OrderToInvoiceCreate] Saving invoice...");
    try {
      // Validation
      ...
      logger.debug("✅ [OrderToInvoiceCreate] Invoice saved successfully", { invoiceId: invoice.id });
      
      toast.success("Fatura başarıyla oluşturuldu");
      navigate(`/sales-invoices/${invoice.id}`);

    } catch (error) {
      logger.error('❌ [OrderToInvoiceCreate] Error saving invoice:', error);
    ...
    return (
      <div className="max-w-2xl mx-auto mt-12">
        ...
        <div className="mt-4 flex justify-center">
          <Button onClick={() => navigate("/orders/list")} variant="outline">
            Siparişler Sayfasına Dön
          </Button>
        </div>
      </div>
    );
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground/70">
                  Onaylanan siparişten fatura oluşturuyorsunuz
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <AlertTitle className="text-purple-900">
                  Siparişten Fatura Oluşturuluyor
                </AlertTitle>
              ...
              {orderData.total_amount && (
                <Badge variant="outline" className="bg-white text-purple-700 border-purple-200">
                  {formatCurrency(orderData.total_amount, orderData.currency || 'TRY')}
                </Badge>
              )}
            </div>
          </Alert>
...
export default OrderToInvoiceCreate;
