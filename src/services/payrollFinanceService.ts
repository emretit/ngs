...
export async function savePayrollRun(options: SavePayrollRunOptions) {
  ...
  try {
    ...
    if (checkError) {
      logger.error('Payroll run check error:', checkError);
    ...
    if (existingRun) {
      ...
      logger.debug(`✓ Mevcut payroll_run kullanılıyor: ${payrollRunId}`);
      ...
      if (deleteError) {
        logger.error('Old items delete error:', deleteError);
      ...
      if (runError || !newRun) {
        logger.error('Payroll run insert error:', runError);
        throw new Error(`Bordro run oluşturulamadı: ${runError?.message || 'Bilinmeyen hata'}`);
      }

      payrollRunId = newRun.id;
      logger.debug(`✓ Yeni payroll_run oluşturuldu: ${payrollRunId}`);
    }
    ...
    const payrollItems: PayrollItemData[] = employeeCalculations.map(({ employeeId, calculation }) => ({
      ...
      allowances_total: (calculation.allowances_cash || 0) + (calculation.allowances_in_kind || 0),
      advances_total: calculation.advances || 0,
    ...
    if (itemsError || !insertedItems) {
      logger.error('Payroll items insert error:', itemsError);
      throw new Error(`Bordro kalemleri kaydedilemedi: ${itemsError?.message || 'Bilinmeyen hata'}`);
    }

    logger.debug(`✓ ${insertedItems.length} payroll_item kaydedildi`);
    ...
    if (updateError) {
      logger.error('Payroll run status update error:', updateError);
    ...
    logger.error('❌ savePayrollRun error:', error);
    throw error;
  }
}
...
export async function syncPayrollToFinance(payrollRunId: string, userId?: string) {
  try {
    ...
    logger.debug(`✓ ${payrollItems.length} payroll_item Finance'e aktarılacak`);
    ...
    if (entriesError || !insertedEntries) {
      logger.error('Finance entries insert error:', entriesError);
    ...
    logger.debug(`✓ ${insertedEntries.length} finance entry oluşturuldu`);
    ...
    if (updateError) {
      logger.error('Payroll run finance sync status update error:', updateError);
    ...
    logger.error('❌ syncPayrollToFinance error:', error);
    throw error;
  }
}
...
export async function approvePayrollRun(
  ...
  try {
    ...
    logger.error('❌ approvePayrollRun error:', error);
    throw error;
  }
}
