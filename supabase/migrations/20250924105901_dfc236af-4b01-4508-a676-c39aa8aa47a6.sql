-- Add order_rank column for drag-and-drop sorting in task board
ALTER TABLE public.activities 
ADD COLUMN IF NOT EXISTS order_rank TEXT NULL;

-- Add indexes for better task board performance
CREATE INDEX IF NOT EXISTS idx_activities_company_status 
ON public.activities (company_id, type, status);

CREATE INDEX IF NOT EXISTS idx_activities_assignee 
ON public.activities (assignee_id, due_date);

-- Create a view to isolate tasks
CREATE OR REPLACE VIEW public.v_tasks AS 
SELECT * FROM public.activities WHERE type = 'task';

-- Function to generate order_rank when NULL (for stable drag-sorting)
